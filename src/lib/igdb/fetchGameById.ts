...
export default async function fetchGameById(id: string) {
  ...
  try {
    const igdbResponse = await fetch("https://api.igdb.com/v4/games", {
      ...
      body: `fields name, cover.image_id, first_release_date, summary, genres.name, platforms.name, involved_companies.company.name, involved_companies.publisher, involved_companies.developer; where id = ${id};`,
    })
